                        ...
                        if ($.active > 0) {
                            ...
                            result.payload.forEach(function(data){
                                ...
                                $('.F5T2_tbody_a').append("<tr>"+
                                    ...
                                    "<td class='options'>"+source+"</td>"+
                                    "<td align='center' class='options'>" + 
                                    "<button class='access_f5 btn btn-success btn-xs btn-attachment-rcv ' data-docket='" + data.docket_no.toUpperCase() + "' data-id='" + data.id + "' data-petitioner='" + data.petitioner_name + "' data-field_office_id='" + data.field_office_id + "'><i class='fa fa-upload'></i> </button> " +
            ...
            $(".deleteProceedButton").unbind("click").on("click",function(){
                ...
                $.wms.executeExternalPost('/ppa-cmis-api_origin/wsv1/Cmis/F5T8',JSON.stringify(payload)).done(function (result) {
                    ...
                    if(result.status != undefined && result.status == "SUCCESS"){
                        ...
                        var payloadDocket = {
                            "method"    : "fetch_by_docket",
                            "SDOCKETNO" : $(".sel-docket").html(),
                            "SUPVOFFICE" : $.wms.urlParam('field')
                        }
                        console.log(payload)
                        $.wms.executeExternalPost('/ppa-cmis-api_origin/wsv1/Cmis/upsertMasterlist',JSON.stringify(payloadDocket)).done(function (result) {
                            console.log(result)
                            if(result.status == "SUCCESS"){
                                ...
                                $.wms.executeExternalPost('/ppa-cmis-api_origin/wsv1/Cmis/upsertMasterlist',JSON.stringify(payloadUpdate)).done(function (result) {
                                    console.log(result)
                                    if(result.status == "SUCCESS"){
                                        $(".modal-loader").addClass("hidden")
                                        $(".deleteProceedButton").attr('disabled',false)
